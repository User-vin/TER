...
def caracteristic_vector_opponent_color_uint8(path, path_seg):
    ...
    """
    if path.split('\\')[-1] != path_seg.split('\\')[-1]:
        print('EEEERRRRRRRRROOOOOOOOORRRRRRRRR')
    img = cv2.imread(path)
...
def caracteristic_vector_rgb(path, path_seg):
    ...
    hist, edges = np.histogramdd(object_pixels, bins=(16,16,16), range=((0,256),(0,256),(0,256)))
    vec = hist.flatten()
    return vec
...
def caracteristic_vector_gray(path, path_seg):
    ...
    hist, edges = np.histogram(object_pixels, bins=(200), range=(0,256))
    return hist
...
if __name__ == "__main__":
    ...
    imgs_set1 = natsorted(glob(r'C:\Users\scott\OneDrive\Bureau\imgs\*.png'))
    imgs_set1_seg = natsorted(glob(r'C:\Users\scott\OneDrive\Bureau\masks\*.png'))
    
    imgs_set2 = natsorted(glob(r'C:\Users\scott\OneDrive\Bureau\imgs\*.png'))
    imgs_set2_seg = natsorted(glob(r'C:\Users\scott\OneDrive\Bureau\masks\*.png'))
